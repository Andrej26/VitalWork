# OperatorSense Live — Source Code

Complete, annotated source of `operatorsense_live.html`. The file is self-contained: markup, styling and
logic in one document, no build step, no libraries, no dependencies. Opening the file in a browser is
all that is needed to run it.

Companion document: **OperatorSense Live — EDA Ring: Technical Documentation** (hardware, data format,
calibration, usage).

> Line numbers refer to `operatorsense_live.html` as shipped alongside this document. The listing below
> is complete: every line of the file appears in one of the sections, except the structural
> `</head>` and `<script>` wrapper tags that separate them.
...
├── <style>   ...  layout and dark theme                      (lines 7-81)
└── <script>
    ├── calibration        ADC code -> ohms -> microsiemens   (lines 200-263)
    ├── upload config      endpoint/auth defaults + schema     (lines 265-305)
    ├── state              module-level variables              (lines 308-329)
    ├── connection         open/read/close the serial port     (lines 331-398)
    ├── parsing            serial line -> sample object        (lines 400-439)
    ├── raw log            last 200 lines as received          (lines 441-450)
    ├── stats + render     20 Hz refresh loop                  (lines 452-507)
    ├── chart              canvas drawing                      (lines 509-587)
    ├── recording          record / mark / clear               (lines 589-653)
    ├── server upload      settings, payload, POST             (lines 655-859)
    └── ui state           button enable/disable, status text  (lines 861-890)
```
...
```
  serial bytes
      │
      ▼
  readLoop()            reads chunks, splits on CR/LF, keeps the incomplete tail
      │
      ▼
  handleLine(line) ──► appendRaw(line)          raw log
      │
      ├──► parseLine(line)                      "D8730:01[CEE3];CAFE;1068" -> object
      ├──► chartData[]                          rolling window for the graph
      ├──► rateStamps[]                         live sample-rate measurement
      └──► recorded[]                           only while recording is on
                │
                ├──► downloadCsv()              CSV file with converted values
                └──► sendCapture() ──► buildPayload() ──► postPayload()   JSON POST

  render()  every 50 ms: statistics, converted readout, drawChart()

```

**Design rules the code follows**

- **No dependencies.** Everything (chart, CSV writer, HTTP client) is written by hand against browser
  APIs, so the file works offline and cannot break through an external library change.
- **The raw value is never lost.** Conversions are applied on top of the ADC code; the original code
  and the original serial line are carried through to every export.
- **Nothing is sent anywhere by default.** The upload path is inert until an endpoint is configured.

---

## 2. Page structure (HTML)

The document head — character set, viewport and the browser-tab title:
...
The body is static markup; the script only fills in values and toggles state. Every element the
script touches has an `id`.
...
A dark theme built on CSS custom properties: change the values in `:root` to restyle the whole page.
Layout uses flexbox for the toolbars and CSS grid for the stat cards, so it reflows on narrow
windows without media queries.
...
Converts the transmitted ADC code into resistance, and resistance into conductance. The measured
table comes from `CALIBRATION.xlsx`; each ring has its own column and is selected by the ring address
carried in every packet. Values between table points are interpolated, values outside it are
...
## 5. Server-upload configuration

Defaults and the payload schema. Everything here is overridden at runtime by the settings the
operator saves in the browser, so a handover copy can be shipped either blank or pre-filled.
...
All mutable state lives in a handful of module-level variables — there is no framework and no store.
`recorded[]` is the capture buffer; `chartData[]` is a bounded rolling window used only for drawing.
...
Opens the port through the Web Serial API, filtered to the FTDI vendor ID with an unfiltered
fallback. `readLoop()` decodes incoming bytes, splits them on any of CR/LF/CRLF and keeps the
incomplete tail in a buffer until the rest of the line arrives — which is why the undocumented line
terminator does not matter.
...
`parseLine()` takes the value as the integer after the **last** `;`, and treats the leading fields as
optional metadata. This is deliberate: if the packet header is ever interpreted differently, the
measurement still parses.
...
Keeps the last 200 lines exactly as received and auto-scrolls only when the operator is already at
the bottom, so scrolling back to inspect something is not interrupted by new data.
...
## 10. Statistics and the render loop

A single 20 Hz timer drives all screen updates. Incoming data only sets a `dirty` flag; the loop
decides when to recompute and redraw. This keeps the UI responsive regardless of the packet rate.
...
The graph is drawn directly on a `<canvas>`: axis scaling, gridlines, event markers, the signal line
and the highlighted latest point. The canvas is resized for the device pixel ratio so the line stays
...
Record, mark event and clear. Each recording gets a `captureId` used by the server to de-duplicate
retried uploads; clearing the buffer starts a new one.
...
Settings persistence, payload construction and the `POST` itself. Failures are reported in plain
language and never discard recorded data — a failed upload can be retried or exported to CSV instead.
...
Central place where buttons are enabled/disabled and the status line is written, so no other code has
to reason about which controls make sense in which state.
...
| Function | Purpose |
|---|---|
| `adcToOhms(adc, ringAddr)` | ADC code -> resistance in ohms, via the ring's calibration table. |
| `adcInCalibratedRange(adc, ringAddr)` | Whether that value sits inside the measured span. |
| `ohmsToMicroSiemens(ohms)` | Resistance -> conductance. |
| `connect()` / `disconnect()` | Open / close the serial port and reset the live view. |
| `readLoop()` | Continuous read; assembles complete text lines. |
| `parseLine(line)` | Serial line -> `{value, pktId, channel, dongle, ring, raw}`. |
| `handleLine(line)` | Feeds one parsed sample into log, chart, rate meter and capture buffer. |
| `appendRaw(line)` | Raw-log ring buffer with scroll preservation. |
| `render()` | 20 Hz loop: statistics, converted readout, chart. |
| `updateDuration()` / `updateRate()` | Capture duration and measured sample rate. |
| `drawChart()` / `sizeCanvas()` | Canvas rendering and high-DPI sizing. |
| `csvNum(x, decimals)` | Number formatting for CSV (empty cell instead of `NaN`). |
| `downloadCsv()` | Builds and downloads the CSV, attaching events to nearest samples. |
| `newCaptureId()` | Identifier for one recording, used for server-side de-duplication. |
| `loadUploadCfg()` / `persistUploadCfg()` | Read/write the upload settings in `localStorage`. |
| `cfgToForm()` / `formToCfg()` / `syncSchemeFields()` | Settings form binding. |
| `buildPayload(isTest)` | Assembles the JSON capture (or an empty test ping). |
| `uploadHeaders()` | Content type plus the selected authentication header. |
| `postPayload(payload)` | `fetch` with a 30 s timeout and readable error messages. |
| `sendCapture(isTest)` | Full upload flow including busy state and result reporting. |
| `setUploadState()` / `refreshUploadUi()` / `openUploadCfg()` | Upload UI state. |
| `setConnectedUI(on)` / `setStatus(text, kind)` | Global UI state and connection status. |
...
| Change | Where |
|---|---|
| Add a ring, or update calibration | `CALIBRATION_TABLES` ([§4](#4-calibration)). |
| Ship with the server pre-configured | `UPLOAD_DEFAULTS` ([§5](#5-server-upload-configuration)). |
| Different default baud rate | The `<select id="baud">` options ([§2](#2-page-structure-html)). |
| Longer/shorter chart window | `CHART_MAX` ([§6](#6-state)). |
| Different packet format | `parseLine()` only ([§8](#8-parsing)). |
| More CSV columns | `downloadCsv()` header string and row mapping ([§13](#13-server-upload) is the JSON equivalent). |
| Restyle | The `:root` custom properties ([§3](#3-styling-css)). |
