...
/**
 * Uploads a completed session to the BioMetrix Laravel server as one nested JSON bundle
 * (`POST /api/uploads/session`, see `test/BioMetrix_API_Documentation.docx`).
 *
...
class SessionHttpUploader(
    ...
    override suspend fun upload(sessionId: Long): Result<String> = withContext(Dispatchers.IO) {
        try {
            val baseUrl = BuildConfig.BIOMETRIX_BASE_URL.trimEnd('/')
            val apiKey = BuildConfig.BIOMETRIX_API_KEY
            if (baseUrl.isEmpty() || apiKey.isEmpty()) {
                return@withContext Result.failure(
                    IllegalStateException(
                        "Server upload is not configured (missing BIOMETRIX_BASE_URL / BIOMETRIX_API_KEY)."
                    )
                )
            }
